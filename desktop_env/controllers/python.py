...
class PythonController:
    def __init__(self, vm_ip: str,
        ...
        # fixme: this is a hacky way to execute python commands. fix it and combine it with installation of packages
        ...
        for _ in range(self.retry_times):
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        "Failed to get accessibility tree. Status code: %d", response.status_code)
                    logger.info("Retrying to get accessibility tree.")
            except Exception as e:
                logger.error(
                    "An error occurred while trying to get the accessibility tree: %s", e)
                logger.info("Retrying to get accessibility tree.")
            time.sleep(self.retry_interval)
    ...
    def get_file(self, file_path: str) -> Optional[bytes]:
        ...
        for _ in range(self.retry_times):
            try:
                ...
                logger.error(
                    "An error occurred while trying to get the file: %s", e)
                logger.info("Retrying to get file.")
            time.sleep(self.retry_interval)
    ...
    def execute_action(self, action: Dict[str, Any]):
        ...
        parameters = action["parameters"] if "parameters" in action else {
            param: action[param] for param in action if param != 'action_type'}
        ...
        if action_type == "MOVE_TO":
            ...
            if parameters == {} or None:
                ...
                y = parameters["y"]
                self.execute_python_command(
                    f"pyautogui.rightClick(x={x}, y={y})")
            else:
                raise Exception(f"Unknown parameters: {parameters}")
            ...
            if parameters == {} or None:
                ...
                y = parameters["y"]
                self.execute_python_command(
                    f"pyautogui.doubleClick(x={x}, y={y})")
            else:
                raise Exception(f"Unknown parameters: {parameters}")
            ...
            self.execute_python_command(
                "pyautogui.typewrite({:})".format(repr(text)))
    ...
    def start_recording(self):
        ...
        for _ in range(self.retry_times):
            try:
                ...
                logger.error(
                    "An error occurred while trying to start recording: %s", e)
                logger.info("Retrying to start recording.")
            time.sleep(self.retry_interval)
    ...
    def end_recording(self, dest: str):
        ...
        for _ in range(self.retry_times):
            try:
                ...
                logger.error(
                    "An error occurred while trying to stop recording: %s", e)
                logger.info("Retrying to stop recording.")
            time.sleep(self.retry_interval)
    ...
    def get_vm_directory_tree(self, path) -> Optional[Dict[str, Any]]:
        ...
        for _ in range(self.retry_times):
            try:
                response = requests.post(self.http_server + "/list_directory", headers={
